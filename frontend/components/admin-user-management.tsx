...
export default function AdminUserManagement() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch("http://172.20.0.3:5000/api/admin/users", {
        credentials: "include",
  ...
  const fetchRegions = async () => {
    try {
      const token = localStorage.getItem("jwt");
      const response = await fetch("http://172.20.0.3:5000/api/regions", {
        credentials: "include",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://172.20.0.3:5000/api/admin/users/${selectedUser.id}`, {
        method: "PUT",
        headers,
        credentials: "include",
        body: JSON.stringify(formData),
  ...
  const handleDelete = async (userId: number) => {
    ...
    try {
      const token = localStorage.getItem("jwt");
      const response = await fetch(`http://172.20.0.3:5000/api/admin/users/${userId}`, {
        method: "DELETE",
        credentials: "include",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
